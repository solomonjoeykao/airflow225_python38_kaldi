...
function install_airflow_and_providers_from_docker_context_files(){
    ...
    if [[ "${UPGRADE_TO_NEWER_DEPENDENCIES}" != "false" ]]; then
        ...
        pip3 install "${pip_flags[@]}" --upgrade --upgrade-strategy eager \
        ...
        if [[ ${AIRFLOW_CONSTRAINTS_LOCATION} == "/"* ]]; then
            grep -ve '^apache-airflow' <"https://raw.githubusercontent.com/apache/airflow/constraints-main/constraints-3.8.txt" > /tmp/constraints.txt
        else
            # Remove provider packages from constraint files because they are locally prepared
            curl -L "https://raw.githubusercontent.com/apache/airflow/constraints-main/constraints-3.8.txt" | grep -ve '^apache-airflow' > /tmp/constraints.txt
        fi
